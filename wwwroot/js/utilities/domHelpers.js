/* MatchPoint - DOM utilities and shared icon set */
...
MP.dom = (function () {
    ...
    /** Event delegation: handles clicks on elements matching `selector` inside `root`. */
    ...
    function toggle(target, shouldShow) {
        return shouldShow ? show(target) : hide(target);
    }
    ...
    function queryParam(name, fallback) {
        ...
        return value === null || value === '' ? (fallback === undefined ? null : fallback) : value;
    }

    function queryParamInt(name, fallback) {
        var value = parseInt(queryParam(name), 10);
        return isNaN(value) ? (fallback === undefined ? null : fallback) : value;
    }

    /** Updates the address bar without reloading, so filters and pages are shareable. */
    ...
    /** Marks a field invalid and writes the message into its `[data-error-for]` element. */
    ...
    function clearFormErrors(form) {
        qsa('.form-control.is-invalid', form).forEach(function (input) {
            input.classList.remove('is-invalid');
            input.removeAttribute('aria-invalid');
        });
        qsa('[data-error-for]', form).forEach(function (element) {
            element.textContent = '';
    ...
    function applyErrors(form, errors) {
        ...
        Object.keys(errors || {}).forEach(function (field) {
            ...
            if (!first) {
                first = form.querySelector('[name="' + field + '"]');
            }
        });
    ...
    /**
     * Maps ASP.NET ModelState keys (PascalCase) onto the form's input names,
     * so server side validation lands on the right field.
     */
    function applyServerErrors(form, serverErrors) {
        if (!serverErrors) {
            return false;
        }
        ...
        Object.keys(serverErrors).forEach(function (key) {
            ...
            var name = key.indexOf('.') > -1 ? key.split('.').pop() : key;
            var camel = name.charAt(0).toLowerCase() + name.slice(1);

            if (form.querySelector('[name="' + camel + '"]')) {
                mapped[camel] = message;
                matched = true;
            } else if (form.querySelector('[name="' + name + '"]')) {
                mapped[name] = message;
                matched = true;
            }
        });
    ...
    var ICONS = {
        ...
        settings: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.6a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9c.14.3.22.63.22.97"/></svg>',
        ...
        sparkles: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3 1.9 5.6L19.5 10l-5.6 1.9L12 17.5l-1.9-5.6L4.5 10l5.6-1.4L12 3Z"/><path d="M19 15l.8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8L19 15Z"/></svg>',
